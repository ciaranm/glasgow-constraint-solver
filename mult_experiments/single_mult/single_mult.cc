...
auto run_mult_test(pair<int, int> v1_range, pair<int, int> v2_range, pair<int, int> v3_range, TestType test_type, const string & proof_prefix) -> bool
{
    ...
    case DC_PROOFS: {
        ...
        stats = solve(
            p, [&](const CurrentState &) -> bool { return false; }, make_optional(ProofOptions{proof_prefix + "_dc"}));
        cout << stats.solve_time.count() << ",";
...
auto run_single() -> int
{
    ...
    p.add_presolver(ProofAutoTable{{v1, v2, v3}});
    solve(
        p, [&](const CurrentState &) -> bool { return false; }, make_optional(ProofOptions{"mult_bc"}));
