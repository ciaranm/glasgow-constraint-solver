...
auto main(int argc, char * argv[]) -> int
{
    ...
    try {
        options.add_options("Program Options")                               //
            ...
            ("proof-files-basename", "Basename for the .opb and .pbp files", //
                cxxopts::value<string>()->default_value("tour"))             //
    ...
    vector<vector<int>> distance = {{0, -1, -1, -1, 83, 71, 22, -1, -1, -1, 69, -1, 76, 35, 14, 2, -1, -1, -1, 19},
    ...
    for (int loc1 = 0; loc1 < n; loc1++) {
        for (int loc2 = 0; loc2 < n; loc2++) {
            p.post(LessThanIf{ConstantIntegerVariableID{Integer{distance[loc1][loc2]}}, max_leg, succ[loc1] == Integer{loc2}});
        }
    }
