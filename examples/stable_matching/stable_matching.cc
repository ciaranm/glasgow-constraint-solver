...
auto main(int argc, char * argv[]) -> int
{
    ...
    for_each_with_index(pairings, [&](auto d, auto dx) {
        ...
        for (unsigned i = 0; i < size; ++i) {
            allocations[dx * 2].push_back(p.create_integer_variable(
                0_i, Integer(size - 1), "a" + to_string(d1) + "_" + to_string(d2) + "_" + to_string(i)));
            allocations[dx * 2 + 1].push_back(p.create_integer_variable(
                0_i, Integer(size - 1), "a" + to_string(d2) + "_" + to_string(d1) + "_" + to_string(i)));
        }
    });
        ...
        auto impose = [&](
            ...
            for (unsigned l = 0; l < size; ++l) {
                for (unsigned l_pref = 0; l_pref < size; ++l_pref) {
                    ...
                    p.post(EqualsIff{
                        right[left_prefs[l][l_pref].raw_value],
                        constant_variable(Integer(right_ranks[left_prefs[l][l_pref].raw_value][l])),
                        link == 1_i});
                }
            }
