...
namespace gcs::bench
{
    ...
    // deadline; it is asked to stop as soon as the solve returns. Uses a plain
    // std::thread + atomic stop flag rather than std::jthread/std::stop_token, so
    // it builds on older libc++ (macOS) too.
    [[nodiscard]] inline auto solve_with_timeout(
        double timeout_seconds, Problem & problem, SolveCallbacks callbacks, const std::optional<ProofOptions> & proof = std::nullopt) -> Stats
    {
        ...
        std::thread timer([&abort_flag, &solve_finished, timeout_seconds] {
            const auto deadline = std::chrono::steady_clock::now() + std::chrono::duration<double>(timeout_seconds);
            while (! solve_finished.load() && std::chrono::steady_clock::now() < deadline)
                std::this_thread::sleep_for(std::chrono::milliseconds(20));
            if (! solve_finished.load())
