...
auto main(int argc, char * argv[]) -> int
{
    ...
    for (int c = 0; c < m; ++c) {
        ...
        while (b == a)
            b = pick_var(rng);
        // The complement of the forbidden set: a value pair is allowed with
        // probability 1 - tightness, so this is the same constraint the negative
        // benchmark expresses, but as an allowed-tuple Table.
