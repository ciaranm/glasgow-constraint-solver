    ...
    if (! vertical_xvs.empty()) {
        for (int c = 0; c < n; ++c)
            for (int r = 0; r < n - 1; ++r)
                ...
                case N: break;
                case V: p.post(LinearEquality{WeightedSum{} + 1_i * grid[r][c] + 1_i * grid[r + 1][c], 5_i, consistency::GAC{}}); break;
                case X: p.post(LinearEquality{WeightedSum{} + 1_i * grid[r][c] + 1_i * grid[r + 1][c], 10_i, consistency::GAC{}}); break;
                case O:
                    auto sum = p.create_integer_variable(0_i, Integer{n * 2});
                    p.post(NotEquals{sum, 5_c});
                    p.post(NotEquals{sum, 10_c});
                    p.post(LinearEquality{WeightedSum{} + 1_i * grid[r][c] + 1_i * grid[r + 1][c] + -1_i * sum, 0_i, consistency::GAC{}});
                    break;
                }
        ...
        for (int r = 0; r < n; ++r)
            for (int c = 0; c < n - 1; ++c)
                ...
                case N: break;
                case V: p.post(LinearEquality{WeightedSum{} + 1_i * grid[r][c] + 1_i * grid[r][c + 1], 5_i, consistency::GAC{}}); break;
                case X: p.post(LinearEquality{WeightedSum{} + 1_i * grid[r][c] + 1_i * grid[r][c + 1], 10_i, consistency::GAC{}}); break;
                case O:
                    auto sum = p.create_integer_variable(0_i, Integer{n * 2});
                    p.post(NotEquals{sum, 5_c});
                    p.post(NotEquals{sum, 10_c});
                    p.post(LinearEquality{WeightedSum{} + 1_i * grid[r][c] + 1_i * grid[r][c + 1] + -1_i * sum, 0_i, consistency::GAC{}});
                    break;
                }
    }
