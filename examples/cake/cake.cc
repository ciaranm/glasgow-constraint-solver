...
auto main(int argc, char * argv[]) -> int
{
    ...
    auto inv_profit = p.create_integer_range_variable(-107500_i, 0_i, "inv_profit");
    p.post(LinearEquality{ Linear{ { 400_i, banana }, { 450_i, chocolate }, { 1_i, inv_profit } }, 0_i });

    p.branch_on(vector<IntegerVariableID>{ banana, chocolate });
    p.minimise(inv_profit);
    auto stats = solve(p, [&] (const State & s) -> bool {
            cout << "banana cakes = " << s(banana) << ", chocolate cakes = " << s(chocolate) << ", profit = " << -s(inv_profit) << endl;
            return true;
            });
