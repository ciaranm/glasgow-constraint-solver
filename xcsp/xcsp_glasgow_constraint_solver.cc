...
namespace
{
    ...
    public:
        ...
        auto buildConstraintMDD(string, vector<XVariable *> & x_vars, vector<XTransition> & transitions) -> void override
        {
            auto vars = need_variables(x_vars);
            ...
            _problem.post(MDD{vars, std::move(layer_transitions), std::move(nodes_per_layer), std::move(accepting_terminals)});
        }
        ...
        auto buildConstraintBinPacking(string, vector<XVariable *> & x_vars, vector<int> & sizes, XCondition & cond) -> void override
        {
        ...
        auto buildConstraintBinPacking(string, vector<XVariable *> & x_vars, vector<int> & sizes, vector<int> & capacities_or_loads, bool load)
            -> void override
        {
            auto items = need_variables(x_vars);
            vector<Integer> sizes_i;
        ...
        auto buildConstraintBinPacking(
            string, vector<XVariable *> & x_vars, vector<int> & sizes, vector<XVariable *> & capacities_or_loads, bool load) -> void override
        {
            if (! load)
