...
class TestGlasgowConstraintSolver(unittest.TestCase):
    ...
    def test_alldifferent_no_solutions(self):
        w = self.gcs.create_integer_variable(range(1, 4), "w")
    ...
    def test_plus(self):
        w = self.gcs.create_integer_variable([1], "w")
        self.gcs.post_arithmetic(w, w, self.x, "sum");
        self.gcs.solve()
        self.assertEqual(self.gcs.get_solution_value(self.x), 2)

    def test_times(self):
        w = self.gcs.create_integer_variable([4], "w")
        p = self.gcs.create_integer_variable([5], "w")
        q = self.gcs.create_integer_variable(range(-100, 100), "w")
        self.gcs.post_arithmetic(w, p, q, "mul");
        self.gcs.solve()
        self.assertEqual(self.gcs.get_solution_value(q), 20)

    def test_div(self):
        w = self.gcs.create_integer_variable([50], "w")
        p = self.gcs.create_integer_variable([2], "w")
        q = self.gcs.create_integer_variable(range(-100, 100), "w")
    ...
    def test_mod(self):
        w = self.gcs.create_integer_variable([11], "w")
        p = self.gcs.create_integer_variable([2], "w")
        q = self.gcs.create_integer_variable(range(-100, 100), "w")
    ...
    def test_count(self):
        w = self.gcs.create_integer_variable([3], "w")
        c = self.gcs.create_integer_variable([2], "c")
        self.gcs.post_count([self.x, self.y, self.z], w, c)
        self.gcs.solve();
        sols = [self.gcs.get_solution_value(self.x),
    ...
    def test_element(self):
        w = self.gcs.create_integer_variable([3], "w")
        c = self.gcs.create_integer_variable([2], "c")
        idx = self.gcs.create_integer_variable([0], "idx")
    ...
    def test_equals_if(self):
        w = self.gcs.create_integer_variable([1], "w")
        reif = self.gcs.create_integer_variable([0], "r")
        self.gcs.post_equals_if(self.x, w, reif)
        self.gcs.maximise(self.x)
        self.gcs.solve()
        self.assertEqual(self.gcs.get_solution_value(self.x), 3)

    def test_not_equals(self):
        c = self.gcs.create_integer_variable([1], "c")
    ...
    def test_or(self):
        b1 = self.gcs.create_integer_variable([0, 1], "b1")
        b2 = self.gcs.create_integer_variable([0], "b2")
    ...
    def test_implies_if(self):
        ...
        b2 = self.gcs.create_integer_variable([0, 1], "b2")
        r = self.gcs.create_integer_variable([0], "r")
        self.gcs.post_implies_if(b1, b2, r)
        self.gcs.minimise(b2)
        self.gcs.solve()
    ...
    def test_min(self):
        w = self.gcs.create_integer_variable([2], "w")
        c = self.gcs.create_integer_variable([1], "c")
        r = self.gcs.create_integer_variable([2], "r")
