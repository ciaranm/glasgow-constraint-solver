...
TEST_CASE("Luby schedule follows the Luby sequence")
{
    ...
    CHECK(first_cutoffs(RestartSchedule::luby(1), 15) == vector<unsigned long long>{1, 1, 2, 1, 1, 2, 4, 1, 1, 2, 1, 1, 2, 4, 8});
}

TEST_CASE("Luby schedule scales every cutoff")
{
    CHECK(first_cutoffs(RestartSchedule::luby(100), 7) == vector<unsigned long long>{100, 100, 200, 100, 100, 200, 400});
}
