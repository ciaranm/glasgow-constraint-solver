...
namespace
{
    ...
    // output). Structurally a port of KnapsackLegacy's knapsack_gac<true>:
        ...
        const Flags & flags, const vector<pair<ProofLine, ProofLine>> & opb_lines, DeadCache & cache) -> void
    {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            for (auto val : state.each_value_mutable(vars[i])) {
                if (! supported_values.contains(val))
                    inference.infer(logger, vars[i] != val, JustifyUsingRUP{hints::Knapsack{owner}}, reason);
            }
        ...
        if (completed_layers.back().empty()) {
            ...
            inference.contradiction(logger, JustifyUsingRUP{hints::Knapsack{owner}}, reason);
            if (emitting)
        ...
        inference.infer_all(logger, inferences, JustifyUsingRUP{hints::Knapsack{owner}}, reason);
        ...
        for (auto layer = completed_layers.rbegin(); layer != completed_layers.rend() && next(layer) != completed_layers.rend();
            ...
            for (auto val : state.each_value_mutable(vars[var_number])) {
                if (! supported.contains(val))
                    inference.infer(logger, vars[var_number] != val, JustifyUsingRUP{hints::Knapsack{owner}}, reason);
            }
        }
