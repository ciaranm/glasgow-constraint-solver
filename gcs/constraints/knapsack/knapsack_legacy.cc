...
KnapsackLegacy::KnapsackLegacy(vector<Integer> weights, vector<Integer> profits, vector<IntegerVariableID> vars, IntegerVariableID weight,
    IntegerVariableID profit) : _coeffs({move(weights), move(profits)}), _vars(move(vars)), _totals({weight, profit})
{
}

KnapsackLegacy::KnapsackLegacy(vector<vector<Integer>> coefficients, vector<IntegerVariableID> vars, vector<IntegerVariableID> totals) :
...
namespace
{
        ...
        const vector<size_t> & undetermined_var_indices, const optional<vector<pair<ProofLine, ProofLine>>> & opb_lines) -> void
    {
        ...
        vector<WPBSum> sums_so_far(coeffs.size());
        ...
        for (const auto & [the_layer_number, the_var_idx] : enumerate(undetermined_var_indices)) {
            ...
            for (auto & [the_sums, the_completed_node_data] : completed_layers.back()) {
                ...
                if constexpr (doing_proof_) {
                    ...
                    WPBSum must_pick_one = WPBSum{} + 1_i * not_in_full_state;
                    WPBSum must_pick_one_val = must_pick_one, must_pick_one_node = must_pick_one;
            ...
            for (auto val : state.each_value_mutable(vars_including_assigned.at(var_idx))) {
                if (! supported_values.contains(val)) {
                    ...
                    inference.infer(logger, vars_including_assigned.at(var_idx) != val, JustifyUsingRUP{hints::KnapsackLegacy{owner}},
        ...
        if (completed_layers.back().empty()) {
            ...
            inference.contradiction(logger, JustifyUsingRUP{hints::KnapsackLegacy{owner}}, eager_reason(generic_reason(reason_variables), state));
        }
        else {
            ...
            inference.infer_all(
                logger, inferences, JustifyUsingRUP{hints::KnapsackLegacy{owner}}, eager_reason(generic_reason(reason_variables), state));
            ...
            for (auto layer = completed_layers.rbegin(); layer != completed_layers.rend() && next(layer) != completed_layers.rend();
                ...
                for (auto val : state.each_value_mutable(var)) {
                    if (! supported.contains(val))
                        inference.infer(
                            logger, var != val, JustifyUsingRUP{hints::KnapsackLegacy{owner}}, eager_reason(generic_reason(reason_variables), state));
                }
            }
        }
    }
        ...
        const vector<pair<ProofLine, ProofLine>> & eqn_lines) -> PropagatorState
    {
        ...
        if (undetermined_vars.empty()) {
            ...
            for (const auto & [x, t] : enumerate(totals)) {
                inference.infer(logger, totals.at(x) == committed_sums.at(x), JustifyUsingRUP{hints::KnapsackLegacy{owner}}, all_vars_assigned);
            }
        }

        for (const auto & [x, v] : enumerate(totals))
            inference.infer(
                logger, v >= committed_sums.at(x), JustifyUsingRUP{hints::KnapsackLegacy{owner}}, eager_reason(generic_reason(vars), state));
...
auto KnapsackLegacy::constraint_type() const -> std::string
{
    return "knapsack_legacy";
}

auto KnapsackLegacy::s_expr(const innards::ProofModel * const model) const -> SExpr
{
