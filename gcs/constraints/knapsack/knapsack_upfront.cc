...
namespace
{
    ...
    auto compute_caps(const State & initial_state, const vector<vector<Integer>> & coeffs, const vector<IntegerVariableID> & vars,
    ...
    auto step(const vector<Integer> & w, Integer v, size_t i, const vector<vector<Integer>> & coeffs, const vector<Integer> & caps,
    ...
    auto build_static_dag(const State & initial_state, const vector<IntegerVariableID> & vars, const vector<vector<Integer>> & coeffs,
        const vector<IntegerVariableID> & totals, const vector<Integer> & caps) -> Dag
    {
        auto n = vars.size();
        auto k = totals.size();
    ...
    auto running_sum(const vector<IntegerVariableID> & vars, const vector<vector<Integer>> & coeffs, size_t x, size_t i) -> WPBSum
    {
        WPBSum s;
    ...
    auto emit_scaffolding(ProofLogger * const logger, const State & initial_state, const vector<IntegerVariableID> & vars,
        const vector<vector<Integer>> & coeffs, size_t k, const Dag & dag, Flags & flags) -> void
    {
        auto n = vars.size();
            ...
            for (const auto & w : dag.nodes_at[i]) {
                for (size_t x = 0; x < k; ++x) {
                    if (! flags.per_coord_flags[i][x].contains(w[x].raw_value)) {
                        auto [g_up, gu_fwd, gu_rev] =
                            logger->create_proof_flag_reifying(running[i][x] >= w[x], format("kpup_{}_{}_{}", i, x, w[x].raw_value), ProofLevel::Top);
                        auto [g_dn, gd_fwd, gd_rev] =
                            logger->create_proof_flag_reifying(running[i][x] <= w[x], format("kpdn_{}_{}_{}", i, x, w[x].raw_value), ProofLevel::Top);
                        flags.per_coord_flags[i][x].emplace(w[x].raw_value, CoordFlags{g_up, gu_fwd, gu_rev, g_dn, gd_fwd, gd_rev});
                    }
                }
            ...
            for (const auto & p : phantoms[i]) {
                for (size_t x = 0; x < k; ++x) {
                    if (! flags.per_coord_flags[i][x].contains(p[x].raw_value)) {
                        auto [g_up, gu_fwd, gu_rev] =
                            logger->create_proof_flag_reifying(running[i][x] >= p[x], format("kpup_{}_{}_{}", i, x, p[x].raw_value), ProofLevel::Top);
                        auto [g_dn, gd_fwd, gd_rev] =
                            logger->create_proof_flag_reifying(running[i][x] <= p[x], format("kpdn_{}_{}_{}", i, x, p[x].raw_value), ProofLevel::Top);
                        flags.per_coord_flags[i][x].emplace(p[x].raw_value, CoordFlags{g_up, gu_fwd, gu_rev, g_dn, gd_fwd, gd_rev});
                    }
                }
                ...
                for (auto val : initial_state.each_value_immutable(vars[i])) {
                    ...
                    for (size_t x = 0; x < k; ++x) {
                        ...
                        logger->emit_rup_proof_line(
                            WPBSum{} + 1_i * ! parent_cf.g_up + 1_i * not_choice + 1_i * succ_cf.g_up >= 1_i, ProofLevel::Top);

                        PolBuilder{}.add(succ_cf.g_dn_rev).add(parent_cf.g_dn_fwd).saturate().emit(*logger, ProofLevel::Top);
                        logger->emit_rup_proof_line(
                            WPBSum{} + 1_i * ! parent_cf.g_dn + 1_i * not_choice + 1_i * succ_cf.g_dn >= 1_i, ProofLevel::Top);
                    }

                    // Per-state forward chain RUP-closes from the per-coord
                    // chains plus the conjunction reification of succ.S.
                    logger->emit_rup_proof_line(WPBSum{} + 1_i * ! parent_s + 1_i * not_choice + 1_i * succ_s >= 1_i, ProofLevel::Top);
                }
            }
        }
        ...
        auto emit_backward_chain = [&](size_t i, const vector<Integer> & succ_w) {
            ...
            for (auto val : initial_state.each_value_immutable(vars[i])) {
                ...
                if (! valid_coords) {
                    logger->emit_rup_proof_line(WPBSum{} + 1_i * ! succ_s + 1_i * not_choice >= 1_i, ProofLevel::Top);
                    continue;
                }
                ...
                for (size_t x = 0; x < k; ++x) {
                    ...
                    logger->emit_rup_proof_line(WPBSum{} + 1_i * ! succ_cf.g_up + 1_i * not_choice + 1_i * parent_cf.g_up >= 1_i, ProofLevel::Top);

                    PolBuilder{}.add(succ_cf.g_dn_fwd).add(parent_cf.g_dn_rev).saturate().emit(*logger, ProofLevel::Top);
                    logger->emit_rup_proof_line(WPBSum{} + 1_i * ! succ_cf.g_dn + 1_i * not_choice + 1_i * parent_cf.g_dn >= 1_i, ProofLevel::Top);
                }

                logger->emit_rup_proof_line(WPBSum{} + 1_i * ! succ_s + 1_i * not_choice + 1_i * parent_s >= 1_i, ProofLevel::Top);
            }
        };
            ...
            for (const auto & p : phantoms[i]) {
                ...
                if (phantom_coord) {
                    ...
                    for (auto u : per_coord_feasible[i][x_star]) {
                        ...
                        if (u > w_phantom) {
                            PolBuilder{}.add(u_cf.g_up_fwd).add(phantom_cf.g_dn_fwd).saturate().emit(*logger, ProofLevel::Top);
                        }
                        else if (u < w_phantom) {
                            PolBuilder{}.add(phantom_cf.g_up_fwd).add(u_cf.g_dn_fwd).saturate().emit(*logger, ProofLevel::Top);
                        }
                    }
                }
    ...
    // being fully logged (in assertion mode the inferences are asserted
    // under the typed hint instead, so the scaffolding would be wasted
    // output). Structurally a port of the default per-call Knapsack's knapsack_gac<true>:
    // walks the static DAG layer by layer under current item domains,
    // emitting per-(parent, val, succ) implication chains, per-parent
    // "must-pick" selectors, per-layer at-least-one constraints, then
    // filters layer-n by current totals domain, then a backward pass that
    // emits ~S for dead states and surfaces unsupported item values. All
    // scaffolding is at ProofLevel::Temporary; flag handles come from the
    // Top-level bridge populated by emit_scaffolding.
    auto propagate(const State & state, auto & inference, ProofLogger * const logger, const ConstraintID & owner,
        const vector<IntegerVariableID> & vars, const vector<vector<Integer>> & coeffs, const vector<IntegerVariableID> & totals, const Dag & dag,
        const Flags & flags, const vector<pair<ProofLine, ProofLine>> & opb_lines, DeadCache & cache) -> void
    {
        auto n = vars.size();
        auto k = totals.size();
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            for (auto val : state.each_value_mutable(vars[i])) {
                if (! supported_values.contains(val))
                    inference.infer(logger, vars[i] != val, JustifyUsingRUP{hints::KnapsackUpfront{owner}}, reason);
            }
        ...
        if (completed_layers.back().empty()) {
            ...
            inference.contradiction(logger, JustifyUsingRUP{hints::KnapsackUpfront{owner}}, reason);
            if (emitting)
        ...
        for (size_t x = 0; x < k; ++x) {
            auto x_capture = x;
            auto [lo_iter, hi_iter] =
                minmax_element(completed_layers.back(), [&](const pair<vector<Integer>, LiveNode> & a, const pair<vector<Integer>, LiveNode> & b) {
            ...
            for (auto v : state.each_value_immutable(totals[x])) {
                if (v > lo && v < hi + 1_i &&
                    none_of(completed_layers.back(), [&](const pair<vector<Integer>, LiveNode> & a) { return a.first[x] == v; }))
            ...
            if (emitting) {
                for (const auto & [w, _] : completed_layers.back()) {
                    ...
                    logger->emit_rup_proof_line_under_reason(
                        eager_reason(reason, state), WPBSum{} + 1_i * ! cf.g_up + 1_i * (totals[x] >= lo) >= 1_i, ProofLevel::Temporary);

                    PolBuilder{}.add(opb_lines[x].second).add(cf.g_dn_fwd).emit(*logger, ProofLevel::Temporary);
                    logger->emit_rup_proof_line_under_reason(
                        eager_reason(reason, state), WPBSum{} + 1_i * ! cf.g_dn + 1_i * (totals[x] <= hi) >= 1_i, ProofLevel::Temporary);
                }
                logger->emit_rup_proof_line_under_reason(
                    eager_reason(reason, state), WPBSum{} + 1_i * (totals[x] >= lo) >= 1_i, ProofLevel::Temporary);
                logger->emit_rup_proof_line_under_reason(
                    eager_reason(reason, state), WPBSum{} + 1_i * (totals[x] <= hi) >= 1_i, ProofLevel::Temporary);
            }
        }

        inference.infer_all(logger, inferences, JustifyUsingRUP{hints::KnapsackUpfront{owner}}, reason);
        ...
        for (auto layer = completed_layers.rbegin(); layer != completed_layers.rend() && next(layer) != completed_layers.rend();
            ...
            for (auto val : state.each_value_mutable(vars[var_number])) {
                if (! supported.contains(val))
                    inference.infer(logger, vars[var_number] != val, JustifyUsingRUP{hints::KnapsackUpfront{owner}}, reason);
            }
        }
