    ...
    -> EncodingData
{
    ...
    // A magnitude channel per operand slot (axis 0 -> "X", 1 -> "Y"), a
    // fresh proof-only magnitude each, even for a repeated operand.
    auto chan1 = product_enc::emit_magnitude_channel(model, initial_state, constraint_id, label_id, "MultiplyBC", v1, 0, "X", naming);
    result.channelling_constraints.insert(
        {v1, ChannellingData{chan1.pos_ge, chan1.pos_le, chan1.neg_ge, chan1.neg_le, true, initial_state.lower_bound(v1) < 0_i}});
    result.mag_var.insert({v1, get<ProofOnlySimpleIntegerVariableID>(chan1.mag)});
    auto chan2 = product_enc::emit_magnitude_channel(model, initial_state, constraint_id, label_id, "MultiplyBC", v2, 1, "Y", naming);
    result.channelling_constraints.insert(
        {v2, ChannellingData{chan2.pos_ge, chan2.pos_le, chan2.neg_ge, chan2.neg_le, true, initial_state.lower_bound(v2) < 0_i}});
    result.mag_var.insert({v2, get<ProofOnlySimpleIntegerVariableID>(chan2.mag)});

    auto grid = product_enc::emit_bit_product_grid(model, constraint_id, label_id, chan1.mag, chan2.mag, naming);
    for (const auto & row : grid.cells) {
        result.initial_bit_products.emplace_back();
        for (const auto & cell : row)
            result.initial_bit_products.back().emplace_back(BitProductData{cell.flag, cell.forwards_reif, cell.reverse_reif, nullopt, nullopt});
    }

    // The two ge0 halves give Sum(prod) == Z; the propagator uses them as the
    // z_eq_product pair (.first: Z >= product, .second: Z <= product), and the
    // product-bound provers discharge the entailed ge0(Z) with the ge0(Z) unit.
    auto zchan = product_enc::emit_result_channel(model, label_id, "MultiplyBC", v3, grid, naming);
    result.v3_eq_product_lines = make_pair(zchan.ge0_ge, zchan.ge0_le);
    result.v3_eq_product_lines_neg = make_pair(zchan.lt0_ge, zchan.lt0_le);
    ...
    result.sign_lines = product_enc::emit_sign_clauses(model, label_id, "MultiplyBC", v1, v2, v3, naming);
