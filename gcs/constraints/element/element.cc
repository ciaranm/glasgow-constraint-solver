...
auto NDimensionalElement<EntryType_, dimensions_>::install_propagators_impl(Propagators & propagators, const IndexVec_ & index_vars) -> void
{
    ...
    //   union removes are there. The index propagators are the only other
    //   propagators of ours that holes in the result affect (there is no
    //   equality propagator without variable entries), and each asks only
    //   whether the entry at some live index tuple is in the result's domain,
    //   which is never a value the union removes.
    //
