...
namespace
{
    ...
    auto prove_not_both(SCCProofContext ctx, long i, long l, long k, const bool using_shifted_pos) -> ProofLine
    {
        ...
        if (using_shifted_pos) {
            ...
            neq_line = ctx.logger.emit_rup_proof_line(
                WeightedPseudoBooleanSum{} + 1_i * (ctx.pos_var_data.at(i).var != Integer{k}) + 1_i * (ctx.pos_var_data.at(i).var != Integer{l}) >= 1_i,
                ProofLevel::Top);
        ...
        if (values.size() > 1) {
            ...
            if (using_shifted_pos) {
                ...
                return ctx.logger.emit_rup_proof_line(
                    WeightedPseudoBooleanSum{} + 1_i * ! (ctx.root_flag_data.shifted_pos_eq[node][idx].flag) >= 0_i,
                    ProofLevel::Top);
            }
            else {
                ctx.logger.emit_proof_comment("AM1 p[" + to_string(node) + "]");
                return ctx.logger.emit_rup_proof_line(
                    WeightedPseudoBooleanSum{} + 1_i * ! (ctx.pos_var_data.at(node).var == Integer{idx}) >= 0_i,
                    ProofLevel::Top);
            }
        }
        else
            ...
            for (long i = 0; i < n; i++) {
                ...
                for (long k = 0; k < n; k++) {
                    ...
                    ctx.logger.emit_rup_proof_line(
                        WeightedPseudoBooleanSum{} + 1_i * ! (ctx.pos_var_data.at(i).var == Integer{j - 1}) +
            ...
            if (i != j) {
                ...
                auto subproof = [&](ProofLogger & logger) {
                    ...
                    for (long k = 0; cmp_less(k, ctx.succ.size()); k++) {
                        ...
                        logger.emit_rup_proof_line(WeightedPseudoBooleanSum{} + 1_i * ! (ctx.pos_var_data.at(i).var == Integer{k}) >= 1_i, ProofLevel::Top);
                    }
                ...
                backwards_reif_line = ctx.logger.emit_red_proof_lines_reverse_reifying(
                    WeightedPseudoBooleanSum{} + 1_i * ctx.pos_var_data.at(j).var + -1_i * ctx.pos_var_data.at(i).var >= 0_i,
    ...
    auto create_shifted_pos(SCCProofContext & ctx, const long i, const long dist)
    {
        ...
        maybe_create_and_emplace_flag_data(ctx.root_flag_data.shifted_pos_geq, i, dist,
            WeightedPseudoBooleanSum{} + 1_i * ctx.pos_var_data.at(i).var + -1_i * ctx.pos_var_data.at(ctx.root).var + Integer{n} * greater_than_flag >= Integer{dist},
        ...
        maybe_create_and_emplace_flag_data(ctx.root_flag_data.shifted_pos_geq, i, dist + 1,
            WeightedPseudoBooleanSum{} + 1_i * ctx.pos_var_data.at(i).var + -1_i * ctx.pos_var_data.at(ctx.root).var + Integer{n} * greater_than_flag >= Integer{dist + 1},
    ...
    auto prove_root_is_0(SCCProofContext & ctx) -> ProofLine
    {
        ...
        if (ctx.root != 0) {
            ...
            line = ctx.logger.emit_rup_proof_line(
                WeightedPseudoBooleanSum{} + 1_i * ctx.root_flag_data.shifted_pos_eq[ctx.root][0].flag >= 1_i,
                ProofLevel::Current);
        }
        else {
            line = ctx.logger.emit_rup_proof_line(
                WeightedPseudoBooleanSum{} + 1_i * (ctx.pos_var_data.at(ctx.root).var == 0_i) >= 1_i,
                ProofLevel::Current);
        }
        ...
        const OrderingAssumption & ordering, const long & val, const Literal & assumption)
    {
        ...
        if (ctx.root != 0) {
            ...
            ctx.logger.emit_rup_proof_line_under_reason(ctx.reason,
                WeightedPseudoBooleanSum{} + 1_i * ! ordering.assumption_flag + 1_i * ! assumption + 1_i * ctx.root_flag_data.shifted_pos_geq[mid][val].flag >= 1_i,
                ProofLevel::Current);
        }
        else {
            ctx.logger.emit_rup_proof_line_under_reason(ctx.reason,
                WeightedPseudoBooleanSum{} + 1_i * (ctx.pos_var_data.at(mid).var >= Integer{val}) >= 1_i,
                ProofLevel::Current);
        }
    }
    ...
    auto prove_pos_and_node_implies_next_node(SCCProofContext & ctx, const long & node, const long & next_node, const long & count)
    {
        ...
        if (ctx.root != 0) {
            ...
            if (next_node != ctx.root) {
                ...
                if (next_node != 0) {
                    ...
                    p_line << ctx.logger.emit_rup_proof_line(
                                  WeightedPseudoBooleanSum{} + 1_i * ! (ctx.succ[node] == Integer{next_node}) + 1_i * ! root_greater_than.at(node).flag >= 1_i,
                                  ProofLevel::Temporary)
                           << " ";
                    p_line << ctx.logger.emit_rup_proof_line(
                                  WeightedPseudoBooleanSum{} + 1_i * ! (ctx.succ[node] == Integer{next_node}) + 1_i * root_greater_than.at(next_node).flag >= 1_i,
                                  ProofLevel::Temporary)
                           << " + ";
                }
                ...
                successor_implies_line = ctx.logger.emit_rup_proof_line_under_reason(ctx.reason,
                    WeightedPseudoBooleanSum{} + 1_i * shifted_pos_eq[next_node][count].flag + 1_i * (ctx.succ[node] != Integer{next_node}) +
                ...
                successor_implies_line = ctx.logger.emit_rup_proof_line(WeightedPseudoBooleanSum{} + 1_i * (! shifted_pos_eq[node][count - 1].flag) + 1_i * (ctx.succ[node] != Integer{next_node}) >= 1_i, ProofLevel::Current);
            }
        }
        else {
            ...
            successor_implies_line = ctx.logger.emit_rup_proof_line(
                WeightedPseudoBooleanSum{} + 1_i * (ctx.pos_var_data.at(node).var != Integer{count - 1}) +
    ...
    auto prove_not_same_val(SCCProofContext & ctx, const long & middle, const long & next_node, const long & count)
    {
        ...
        if (ctx.root != 0) {
            ...
            temp_p_line.add_and_saturate(ctx.logger.emit_rup_proof_line(
                WeightedPseudoBooleanSum{} + -1_i * ctx.pos_var_data.at(next_node).var + 1_i * ctx.pos_var_data.at(middle).var >= Integer{-n + 1}, ProofLevel::Temporary));
            ...
            temp_p_line.add_and_saturate(geq_and_leq);
            temp_p_line.add_and_saturate(ctx.logger.emit_rup_proof_line(
                WeightedPseudoBooleanSum{} + -1_i * ctx.pos_var_data.at(middle).var + 1_i * ctx.pos_var_data.at(next_node).var >= Integer{-n + 1}, ProofLevel::Temporary));
            ...
            ctx.logger.emit_rup_proof_line_under_reason(ctx.reason,
                WeightedPseudoBooleanSum{} + 1_i * ! ctx.flag_data[next_node].greater_than[middle].flag +
                        1_i * ! shifted_pos_eq[middle][count].flag +
                        1_i * (! shifted_pos_eq[next_node][count].flag) >=
                    1_i,
                ProofLevel::Temporary);
        ...
        const Literal & assumption) -> ProofLine
    {
        ...
        if (root != 0) {
            ...
            p_line.add_and_saturate(ctx.logger.emit_rup_proof_line(
                WeightedPseudoBooleanSum{} + 1_i * ctx.flag_data[root].greater_than[last].flag + 1_i * ctx.flag_data[last].greater_than[root].flag >= 1_i,
                ProofLevel::Temporary));
            ...
            exclusion_line = ctx.logger.emit_rup_proof_line_under_reason(ctx.reason,
                WeightedPseudoBooleanSum{} + 1_i * ! assumption + 1_i * ! ordering.assumption_flag + 1_i * ! (ctx.pos_var_data.at(last).var == Integer{count}) >= 1_i,
                ProofLevel::Current);
        }
    ...
    auto prove_skipped_subtree(SCCProofContext & ctx, const long node, const long next_node, const long skipped_subroot)
    {
        ...
        final_contradiction_p_line << ctx.logger.emit_rup_proof_line(
                                          WeightedPseudoBooleanSum{} + 1_i * (ctx.succ[node] != Integer{next_node}) +
        ...
        ctx.logger.emit_rup_proof_line_under_reason(ctx.reason,
            WeightedPseudoBooleanSum{} + 1_i * (ctx.succ[node] != Integer{next_node}) >= 1_i, ProofLevel::Current);
    }
