            ...
            for (std::size_t a = 0; a < m; ++a) {
                ...
                for (std::size_t b = a + 1; b < m; ++b) {
                    ...
                    for (std::size_t j = a; j <= b; ++j) {
                        ...
                        if (lower > lb_j)
                            ...
                            inference.infer(logger, counts[j] >= lower,
                                JustifyExplicitlyThenRUP{[&, a = a, b = b, j = j](const ReasonLiterals &) {
                                    auto & tracker = logger->names_and_ids_tracker();
                                    PolBuilder pb;
                                    for (std::size_t v = a; v <= b; ++v)
                                        if (v != j) {
                    ...
                    if (confined_count > cap) {
                        inference.contradiction(logger, JustifyExplicitlyThenRUP{[&](const ReasonLiterals &) { emit_capacity_pol(nullopt); }}, LazyReasonOver{vars, [&](const State &, ReasonLiterals & out) { out = capacity_reason(nullopt); }});
                    }
                        ...
                        for (const auto & var : vars) {
                            ...
                            for (const auto & val : hall)
                                if (state.in_domain(var, val))
                                    inference.infer(logger, var != val, JustifyExplicitlyThenRUP{[&](const ReasonLiterals &) { emit_capacity_pol(nullopt); }}, LazyReasonOver{vars, [&](const State &, ReasonLiterals & out) { out = capacity_reason(nullopt); }});
                        }
                    }
                    ...
                    if (potential_count < demand) {
                        inference.contradiction(logger, JustifyExplicitlyThenRUP{[&](const ReasonLiterals &) { emit_demand_pol(nullopt, 0_i); }}, LazyReasonOver{vars, [&](const State &, ReasonLiterals & out) { out = demand_reason(nullopt); }});
                    }
                    else if (potential_count == demand) {
                        for (const auto & var : potential)
                            for (const auto & val : state.each_value_mutable(var))
                                if (! hall.contains(val))
                                    inference.infer(logger, var != val, JustifyExplicitlyThenRUP{[&, var = var, val = val](const ReasonLiterals &) { emit_demand_pol(var, val); }}, LazyReasonOver{vars, [&](const State &, ReasonLiterals & out) { out = demand_reason(nullopt); }});
                    }
                }
            }
