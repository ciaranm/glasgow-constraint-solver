...
namespace
{
        ...
        shared_ptr<vector<vector<IntegerVariableID>>> nogood_vars, const vector<IntegerVariableID> & trigger_vars) -> void
    {
        ...
        propagators.install(
            ...
            // literals the store holds by then, so nothing here says what the
            // propagator reads. A nogood may hold x == v, which a removal from
            // x's interior falsifies, so say so explicitly: the nogoods to come
            // may mention any variable they could be learned over.
            Triggers{.interior_reads = trigger_vars});
    }
}
