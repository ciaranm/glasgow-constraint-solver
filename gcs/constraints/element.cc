...
auto Element::convert_to_low_level(LowLevelConstraintStore & p, const State & initial_state) && -> void
{
    if (_vars.empty()) {
        p.cnf( { } );
        return;
    ...
    p.cnf({ _idx_var >= 0_i });
    p.cnf({ _idx_var < Integer(_vars.size()) });
    ...
    p.cnf({ _var < max_upper + 1_i });
    p.cnf({ _var >= min_lower });
    ...
    for_each_with_index(_vars, [&] (auto & v, auto idx) {
        ...
        for ( ; lower <= upper ; ++lower) {
            p.cnf({ _idx_var != Integer(idx), v != lower, _var == lower });
            }
    });
}
