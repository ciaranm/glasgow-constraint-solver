    ...
    const State & state, auto & inference, ProofLogger * const logger) -> void
{
    ...
    while (! to_propagate.empty()) {
        ...
        for (auto other : to_propagate) {
            if (other.second == val) {
                ...
                inference.infer_not_equal(logger, var, val, JustifyUsingRUP{},
                    ReasonFunction{[var = other.first, val = val]() { return ReasonLiterals{{var == val}}; }});
            }
        }
        ...
        while (i != unassigned.end()) {
            auto other = *i;
            if (other != var) {
                inference.infer_not_equal(logger, other, val, JustifyUsingRUP{}, ReasonFunction{[var = var, val = val]() { return ReasonLiterals{{var == val}}; }});
