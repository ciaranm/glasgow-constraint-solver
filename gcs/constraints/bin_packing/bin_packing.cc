...
namespace
{
        ...
        const vector<Integer> & capacities, size_t b) -> long long
    {
        ...
        for (auto & s : sizes)
            total += s.raw_value;
        long long ext = have_loads ? state.upper_bound(loads[b]).raw_value : capacities[b].raw_value;
        return min(total, ext);
    }
    ...
    auto build_dag(const State & state, const vector<IntegerVariableID> & items, const vector<Integer> & sizes, bool have_loads,
        const vector<IntegerVariableID> & loads, const vector<Integer> & capacities, size_t b, long long cap) -> PerBinDag
    {
        auto n = items.size();
        ...
        for (long long w = 0; w <= cap; ++w) {
            bool accept = have_loads ? state.in_domain(loads[b], Integer{w}) : (w <= capacities[b].raw_value);
            if (accept)
                bwd[n].insert(w);
        }
    ...
    auto emit_bin_scaffolding(ProofLogger * logger, size_t b, const vector<IntegerVariableID> & items, const vector<Integer> & sizes,
        const PerBinDag & dag, PerBinFlags & flags) -> void
    {
        ...
        for (size_t i = 0; i <= n; ++i) {
            ...
            for (auto w : dag.nodes_at[i]) {
                auto wI = Integer{w};
                auto [g_up, gu_fwd, gu_rev] = logger->create_proof_flag_reifying(sum_expr >= wI, format("bpup_{}_{}_{}", b, i, w), ProofLevel::Top);
                auto [g_dn, gd_fwd, gd_rev] = logger->create_proof_flag_reifying(sum_expr <= wI, format("bpdn_{}_{}_{}", b, i, w), ProofLevel::Top);
                auto [s, s_fwd, s_rev] =
    ...
    auto run_stage3_for_bin(const State & state, auto & inference, ProofLogger * logger, const vector<IntegerVariableID> & items,
        const vector<Integer> & sizes, const PerBinDag & dag, const PerBinFlags & flags, size_t b, const Reason & reason, const ConstraintID & owner)
        -> void
    {
        auto n = items.size();
        auto bin_idx = Integer{static_cast<long long>(b)};
...
auto BinPacking::prepare(Propagators &, State & initial_state, ProofModel * const) -> bool
{
    ...
    if (! _bounds_only) {
        ...
        for (size_t b = 0; b < num_bins; ++b) {
            ...
            _bridge->dags.push_back(build_dag(initial_state, _items, _sizes, _have_loads, _loads, _capacities, b, cap));
        }
    }
...
auto BinPacking::install_propagators(Propagators & propagators) -> void
{
    ...
    if (! _bounds_only) {
        ...
        // In assertion mode the propagator's prunes are asserted under the
        // typed hint instead of RUP-derived, so the scaffolding would be
        // wasted output and is skipped.
        propagators.install_initialiser([items = _items, sizes = _sizes, bridge = _bridge](State &, auto &, ProofLogger * const logger) -> void {
            if (! logger || logger->get_assertion_level() != AssertionLevel::Off)
                return;
            for (size_t b = 0; b < bridge->dags.size(); ++b)
                emit_bin_scaffolding(logger, b, items, sizes, bridge->dags[b], bridge->flags[b]);
        });
    }
    ...
    propagators.install(
        ...
        [items = _items, sizes = _sizes, loads = _loads, capacities = _capacities, have_loads = _have_loads, bounds_only = _bounds_only,
            ...
            if (! bounds_only && bridge) {
                ...
                for (size_t b = 0; b < num_bins; ++b)
                    run_stage3_for_bin(state, inference, logger, items, sizes, bridge->dags[b], bridge->flags[b], b, reason, owner);
            }
