...
namespace
{
    ...
    auto initialise_graph(MDDGraph & graph, const vector<IntegerVariableID> & vars, const vector<long> & nodes_per_layer,
        ...
        if (logger && logger->get_assertion_level() == AssertionLevel::Off) {
            ...
            for (const auto & q : graph.nodes[num_vars])
                if (! reachable_accepting.contains(q))
                    log_additional_inference(logger, {}, {! state_at_pos_flags[num_vars][q]}, state, reason, "non-accepting terminal");
        }
        ...
        if (graph.out_deg[i][k] == 0 && i > 0) {
            for (const auto & edge : graph.in_edges[i][k]) {
                ...
                for (const auto & val : edge.second) {
                    ...
                    if (logger)
                        log_additional_inference(logger, {vars[i - 1] != val}, {! state_at_pos_flags[i - 1][l]}, state, reason, "dec outdeg inner");
        ...
        ProofLogger * const logger, const ConstraintID & owner) -> void
    {
        ...
        if (! graph.initialised)
            initialise_graph(graph, vars, nodes_per_layer, layer_transitions, accepting_terminals, state_at_pos_flags, state, reason, logger);
            ...
            for (const auto & val : _opb_alphabet[idx]) {
                ...
                if (new_q == -1) {
                    ...
                    model.add_constraint(
                        WPBSum{} + 1_i * ! _state_at_pos_flags[idx][q] + 1_i * (_vars[idx] != val) + 1_i * _state_at_pos_flags[idx + 1][new_q] >=
                        1_i);
                }
            }
        }
...
auto MDD::install_propagators(Propagators & propagators) -> void
{
    ...
    propagators.install(
        constraint_id(),
        [v = _vars, t = _layer_transitions, npl = _nodes_per_layer, ats = _accepting_terminals, g = _graph_idx, flags = move(_state_at_pos_flags),
            owner = constraint_id()](const State & state, auto & inference, ProofLogger * const logger) -> PropagatorState {
            propagate_mdd(v, npl, t, ats, flags, g, state, inference, logger, owner);
