...
namespace
{
        ...
        -> DerivedPBConstraint
    {
        ...
        auto z_largest_power = Integer{1 << (logger.names_and_ids_tracker().num_bits(z) - 1)};
        ...
        -> void
    {
        ...
        if (z_min <= 0_i && z_max >= 0_i && y_min <= 0_i && y_max >= 0_i) {
            ...
            inference.infer(logger, x_var < largest_possible_quotient + 1_i,
                or_use_rup_if(JustifyExplicitly{lower_justf}, use_rup), [lits = Literals{z_var >= var_bounds.at(z_var).first, z_var < var_bounds.at(z_var).second + 1_i, y_var >= var_bounds.at(y_var).first, y_var < var_bounds.at(y_var).second + 1_i}]() { return lits; });
            ...
            inference.infer(logger, x_var >= smallest_possible_quotient,
                or_use_rup_if(JustifyExplicitly{upper_justf}, use_rup), [lits = Literals{z_var >= var_bounds.at(z_var).first, z_var < var_bounds.at(z_var).second + 1_i, y_var >= var_bounds.at(y_var).first, y_var < var_bounds.at(y_var).second + 1_i}]() { return lits; });
        }
        else if (y_min == 0_i && y_max != 0_i && (z_min > 0_i || z_max < 0_i)) {
            ...
            filter_quotient(x_var, y_var, z_var, z_min, z_max, 1_i, y_max, all_vars, state,
                inference, channelling_constraints, mag_var, z_eq_product_lines, logger, bit_products, x_is_first, use_rup);
        }
        else if (y_min != 0_i && y_max == 0_i && (z_min > 0_i || z_max < 0_i)) {
            ...
            filter_quotient(x_var, y_var, z_var, z_min, z_max, y_min, -1_i, all_vars, state, inference,
                channelling_constraints, mag_var, z_eq_product_lines, logger, bit_products, x_is_first, use_rup);
        }
            ...
            if (smallest_possible_quotient > largest_possible_quotient) {
                inference.infer(logger, FalseLiteral{}, or_use_rup_if(JustifyExplicitly{both_justf}, use_rup),
                ...
                inference.infer(logger, x_var < largest_possible_quotient + 1_i,
                    or_use_rup_if(JustifyExplicitly{upper_justf}, use_rup),
                    [lits = Literals{z_var >= var_bounds.at(z_var).first, z_var < var_bounds.at(z_var).second + 1_i,
                         y_var >= var_bounds.at(y_var).first, y_var < var_bounds.at(y_var).second + 1_i}]() { return lits; });
                inference.infer(logger, x_var >= smallest_possible_quotient,
                    or_use_rup_if(JustifyExplicitly{lower_justf}, use_rup),
                    [lits = Literals{z_var >= var_bounds.at(z_var).first, z_var < var_bounds.at(z_var).second + 1_i,
                         y_var >= var_bounds.at(y_var).first, y_var < var_bounds.at(y_var).second + 1_i}]() { return lits; });
            }
        }
...
auto MultBC::install(Propagators & propagators, State & initial_state, ProofModel * const optional_model) && -> void
{
    ...
    if (optional_model) {
        ...
        auto make_magnitude_term = [&](SimpleIntegerVariableID & v, const string & name)
            ...
            if (initial_state.lower_bound(v) < 0_i) {
                ...
                auto num_bits = optional_model->names_and_ids_tracker().num_bits(v);
    ...
    propagators.install([v1 = _v1, v2 = _v2, v3 = _v3, bit_products_h = bit_products_handle,
                            ...
                            use_rup = _use_gac_justifications](const State & state, auto & inference,
        ...
        do {
            ...
            inference.infer_all(logger, {v3 < largest_product + 1_i, v3 >= smallest_product},
                or_use_rup_if(JustifyExplicitly{justf}, use_rup),
            ...
            filter_quotient(v1, v2, v3, bounds3.first, bounds3.second, bounds2.first, bounds2.second, all_vars, state, inference,
                channelling_constraints, mag_var, v3_eq_product_lines, logger, bit_products, true, use_rup);
            ...
            filter_quotient(v2, v1, v3, bounds3.first, bounds3.second, bounds1.first, bounds1.second, all_vars, state, inference,
                channelling_constraints, mag_var, v3_eq_product_lines, logger, bit_products, false, use_rup);
