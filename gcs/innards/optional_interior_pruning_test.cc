 ...
 * analyse_optional_interior_pruning(): which variables' holes affect each
 * propagator, and the least-fixpoint question of which optional prunings
 * something could observe. The first half drives the analysis with synthetic
 * propagators, which is the only way to reach every case (every variable
 * Element is conditionally affected by, its index propagator is affected by
 * unconditionally, so it cannot exercise promotion on its own). The second
 * half builds the two constant-array Element models the analysis was designed
 * around, qap and tsp, and checks it says what issue #902 says it should. */
...
namespace
{
    ...
    // propagator is affected by holes in `pruning_hole_sensitivity`, and whose
    // fallback is affected only by the bounds of `targets` unless told
    // otherwise.
    auto install_pair(Propagators & propagators, unsigned long long constraint, const vector<IntegerVariableID> & targets,
        const vector<IntegerVariableID> & pruning_hole_sensitivity, const vector<IntegerVariableID> & fallback_hole_sensitivity = {}) -> void
    {
        Triggers pruning_triggers{.on_change = pruning_hole_sensitivity, .on_bounds = targets};
        Triggers fallback_triggers{.on_change = fallback_hole_sensitivity, .on_bounds = targets};
...
TEST_CASE("An optional pruning whose holes affect nothing else is not needed")
{
    State state;
    ...
    // Another constraint affected only by x's bounds, or only by whether it is
...
TEST_CASE("Another constraint that holes in a target affect makes the pruning needed")
{
    State state;
    auto x = state.allocate_integer_variable_with_state(0_i, 9_i);
    Stats stats;
...
TEST_CASE("A constraint's own sensitivity to its targets does not make its pruning needed")
{
    State state;
    auto x = state.allocate_integer_variable_with_state(0_i, 9_i);
    Stats stats;
    ...
    // Element's shape: an always-on propagator of the same constraint that
    // holes in the target affect. That is exempt; the pair's declaration
    // promises it cannot tell the difference.
...
TEST_CASE("Hole sensitivity is derived from the triggers")
{
    State state;
    auto x = state.allocate_integer_variable_with_state(0_i, 9_i);
    Stats stats;
...
TEST_CASE("Triggers::holes_affect_propagation overrides what the triggers say")
{
    State state;
    auto x = state.allocate_integer_variable_with_state(0_i, 9_i);
    Stats stats;

    SECTION("an empty list: woken on any change, but affected only by bounds")
    {
        Propagators propagators{stats};
        install_pair(propagators, 1, {x}, {});
        propagators.install(
            NumberedConstraint{2}, does_nothing(), Triggers{.on_change = {x}, .holes_affect_propagation = vector<IntegerVariableID>{}});
        CHECK_FALSE(needed(propagators, 1));
    }

    SECTION("an explicit entry for a variable only watched for its bounds")
    {
        Propagators propagators{stats};
        install_pair(propagators, 1, {x}, {});
        propagators.install(
            NumberedConstraint{2}, does_nothing(), Triggers{.on_bounds = {x}, .holes_affect_propagation = vector<IntegerVariableID>{x}});
        CHECK(needed(propagators, 1));
    }
}
...
TEST_CASE("Views resolve to their underlying variable, and constants have no interior")
{
    ...
    SECTION("an observer of a view of a target")
    {
        Propagators propagators{stats};
        install_pair(propagators, 1, {x}, {});
...
TEST_CASE("Permanently disabled propagators are affected by nothing")
{
    State state;
    auto x = state.allocate_integer_variable_with_state(0_i, 9_i);
    Stats stats;

    SECTION("a retired observer")
    {
        Propagators propagators{stats};
        install_pair(propagators, 1, {x}, {});
        propagators.install(NumberedConstraint{2}, does_nothing(), Triggers{.on_change = {x}});
    ...
    SECTION("a retired pair is not reported, and its fallback's sensitivity does not count")
    {
        Propagators propagators{stats};
        install_pair(propagators, 1, {x}, {}, {x});
...
TEST_CASE("A needed pruning's own sensitivity promotes further pruning")
{
    State state;
    auto x = state.allocate_integer_variable_with_state(0_i, 9_i);
    auto y = state.allocate_integer_variable_with_state(0_i, 9_i);
    ...
    // 1 prunes x, and holes in y affect it only when it does; 2 prunes y, holes
    // in z affecting it; 3 prunes z. Nothing is affected by x yet, so nothing is
    // needed.
    ...
    // Now something is affected by x, and the whole chain switches on, each
    // link blamed on the one before it.
...
TEST_CASE("A cycle of prunings that nothing else reaches stays off")
{
    ...
    // Each could observe the other, so switching off whatever nothing is
    // affected by, starting with both on, would keep both. The least fixpoint
    // has both off.
    SECTION("unreached")
    {
        Propagators propagators{stats};
...
TEST_CASE("A fallback's hole sensitivity counts whether or not its pruning is needed")
{
    State state;
    auto x = state.allocate_integer_variable_with_state(0_i, 9_i);
    auto y = state.allocate_integer_variable_with_state(0_i, 9_i);
...
TEST_CASE("qap: nothing can observe an element result's interior")
{
    ...
    // Holes in the index variables, on the other hand, do affect something: the
...
TEST_CASE("An element result's interior is observed only by constraints that can see holes")
{
    ...
    SECTION("an element whose result is another element's array entry")
    {
        ...
        // propagator is affected by holes in y, so e2's pruning is needed. e1
        // itself declares no pair at all, since its array has variable entries.
        Problem p;
