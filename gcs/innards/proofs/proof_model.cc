...
auto ProofModel::create_proof_flag(const string &) -> ProofFlag
{
    ProofFlag result{variable_constraints_tracker().allocate_flag_index(), true};
    string name = variable_constraints_tracker().rewrite_variable_name("f" + to_string(result.index)); // + "_" + n);
    variable_constraints_tracker().track_flag(result, name);
    return result;
}
