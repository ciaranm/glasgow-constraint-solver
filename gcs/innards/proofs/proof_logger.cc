...
namespace
{
    auto witness_literal(VariableConstraintsTracker & variable_constraints_tracker, const ProofLiteralOrFlag & lit) -> string
    {
        return overloaded{
...
auto ProofLogger::create_proof_flag_reifying(const SumLessEqual<Weighted<PseudoBooleanTerm>> & ineq,
    const string &, ProofLevel level) -> tuple<ProofFlag, ProofLine, ProofLine>
{
    ProofFlag flag{variable_constraints_tracker().allocate_flag_index(), true};
    string name = variable_constraints_tracker().rewrite_variable_name("f" + to_string(flag.index)); // + "_" + n);
    variable_constraints_tracker().track_flag(flag, name);
