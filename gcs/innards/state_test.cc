...
TEST_CASE("State infers >=")
{
    ...
    SECTION("var >= value")
    {
        CHECK(state.infer(var >= 3_i, NoJustificationNeeded{}) == Inference::Change);
        check_range(state, var, 3_i, 10_i);
    }

    SECTION("var + offset >= value")
    {
        CHECK(state.infer(var + 1_i >= 5_i, NoJustificationNeeded{}) == Inference::Change);
        check_range(state, var, 4_i, 10_i);
    }

    SECTION("-var + offset >= value")
    {
        CHECK(state.infer(-var + 1_i >= -7_i, NoJustificationNeeded{}) == Inference::Change);
...
TEST_CASE("State infers <")
{
    ...
    SECTION("var < value")
    {
        CHECK(state.infer(var < 7_i, NoJustificationNeeded{}) == Inference::Change);
        check_range(state, var, 1_i, 6_i);
    }

    SECTION("var + offset < value")
    {
        CHECK(state.infer(var + 1_i < 4_i, NoJustificationNeeded{}) == Inference::Change);
        check_range(state, var, 1_i, 2_i);
    }

    SECTION("-var + offset < value")
    {
        CHECK(state.infer(-var + 1_i < -2_i, NoJustificationNeeded{}) == Inference::Change);
        check_range(state, var, 4_i, 10_i);
    }
}
