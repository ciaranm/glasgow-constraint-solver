...
TEST_CASE("State infers >=")
{
    ...
    SECTION("var >= value")
    {
        CHECK(state.infer(var >= 3_i) == HowChanged::BoundsChanged);
        check_range(state, var, 3_i, 10_i);
    }

    SECTION("var + offset >= value")
    {
        CHECK(state.infer(var + 1_i >= 5_i) == HowChanged::BoundsChanged);
        check_range(state, var, 4_i, 10_i);
    }

    SECTION("-var + offset >= value")
    {
        CHECK(state.infer(-var + 1_i >= -7_i) == HowChanged::BoundsChanged);
...
TEST_CASE("State infers <")
{
    ...
    SECTION("var < value")
    {
        CHECK(state.infer(var < 7_i) == HowChanged::BoundsChanged);
        check_range(state, var, 1_i, 6_i);
    }

    SECTION("var + offset < value")
    {
        CHECK(state.infer(var + 1_i < 4_i) == HowChanged::BoundsChanged);
        check_range(state, var, 1_i, 2_i);
    }

    SECTION("-var + offset < value")
    {
        CHECK(state.infer(-var + 1_i < -2_i) == HowChanged::BoundsChanged);
        check_range(state, var, 4_i, 10_i);
    }
}
...
TEST_CASE("State infers !=")
{
    ...
    SECTION("var != value")
    {
        CHECK(state.infer(var != 7_i) == HowChanged::InteriorValuesChanged);
    ...
    SECTION("var + offset != value")
    {
        CHECK(state.infer(var + 1_i != 7_i) == HowChanged::InteriorValuesChanged);
        CHECK(state.bounds(var) == pair{1_i, 10_i});
        for (auto i = 1_i; i <= 10_i; ++i)
...
TEST_CASE("State infers =")
{
    ...
    SECTION("var = value")
    {
        CHECK(state.infer(var == 7_i) == HowChanged::Instantiated);
    ...
    SECTION("var + offset = value")
    {
        CHECK(state.infer(var + 1_i == 7_i) == HowChanged::Instantiated);
