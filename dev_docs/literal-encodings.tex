...
in the OPB.  The solver derives it, one cutting-planes step per bound at the top
of the proof; \cref{rem:derivedbounds} says how.

\begin{remarkN}[Where the view bounds come from]
\label{rem:derivedbounds}
For $B\in[0,4]$ and $W=B+10$, $\BinEnc(W)\geq 10$ is the sum of the view-link
row $\BinEnc(W)-\BinEnc(B)\geq 10$ with $\BinEnc(B)\geq 0$, and
$-\BinEnc(W)\geq -14$ the sum of the other view-link row with
$-\BinEnc(B)\geq -4$.  Every $\BinEnc(B)$ term cancels, the two rows carrying
$B$'s bit coefficients with opposite signs, so each sum is $\Bnd(W)$'s row
exactly and no saturation is wanted.  With $s_W=-1$ the pairing swaps, the
view's bounds coming from the base's the other way round.

The solver used to write these two as OPB rows instead, next to the
\ViewLink{} rows that imply them, so that the OPB carried two rows per view
variable that were a consequence of two other rows of the same OPB.  Deriving
them is better for the reason a proof-only variable introduced inside the proof
is better than one asserted into the model: the OPB is then the model and
nothing else, and a checker that re-derives the OPB from the CP model has
nothing extra to reproduce.  The codebase already draws that distinction ---
...
view variables now use it.

Two things have to be got right.  The derivation has to precede every boundary
pin on $W$, since a pin is a RUP line and would otherwise have nothing to
propagate from; \code{need\_view} therefore queues it before it lets anything
create an atom of $W$.  And the pin has to stay RUP-derivable, which it is once
$\Bnd(W)$ is a line over $W$'s own bit vector, but would not be if it had to
cross $\ViewLink(W)$ instead --- that configuration is the one discussed after
\cref{eq:inlink}, and it is not RUP.

An underlying variable that is itself a free bit-sum, with no bound rows of its
own because its bounds are not a model consequence either, has nothing for these
sums to add to.  A view of one is exempt from \inv{Bound} exactly as the
underlying is, and gets neither bound lines nor boundary pins.  Nothing takes a
view of such a variable today.
\end{remarkN}
...
ones included, because $\Bnd(W)$ is in $F_t$ for each --- derived at the top of
the proof, ahead of any atom of $W$, per \cref{rem:derivedbounds}.
...
In the other direction the implementation used to be \emph{stronger} than
\inv{Bound} asks, and that cost measurable proof.  It emitted a pin for every
out-of-range threshold it created, not one a side, so a variable accumulated
hundreds of top-of-proof units that \cref{lem:chain} supplies from one.  On the
TSP example this was $2{,}993$ pins over $68$ distinct variable-and-side pairs,
the worst single pair carrying $315$ --- a cost variable with lower bound $0$
whose thresholds run from $\atge{w}{-531}$ up to $\atge{w}{0}$, each pinned
separately.

\code{ensure\_boundary\_pin} now emits one a side, matching the invariant, and
the same run writes $68$ pins.  Pinning the extreme threshold \emph{so far
defined} would not achieve this, because thresholds are created one at a time
and in no particular order: on the lower side the invariant wants the largest,
and a stream arriving in increasing order needs a fresh pin at each.  The
threshold at the definition bound is the largest value the invariant can ask
for, which is what makes pinning there a one-off.  Two details make it work.
The pin has to stay a persistent top-of-proof line rather than one re-derived
per use, for the reason \cref{sec:enumeration} gives; and the threshold at the
definition bound is one the model need never have asked for, so it is created
\emph{inside the proof}, by redundance, on the first out-of-range request.
Creating it while the OPB is still being written would put two rows there that
\code{cake\_pb\_cp}, working from the same \code{.scp}, has no reason to create,
and the solver's proof is then checked against an OPB in which that threshold's
