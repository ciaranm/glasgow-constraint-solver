...
| `hitori --dzn h5-1.dzn --connectivity decomposition` vs `--connectivity propagator` | **The largest gap in the set**, and *not* a same-search pair: the stdlib connectivity decomposition searches 113 671 recursions for **815 MB / 796.3 s**, the native `Reachable` propagator 24 for **0.25 MB / 0.14 s**. Both axes move the same way, which is the unusual case — the encodings differ in kind, not just in strength (see [`connectivity-proofs.md`](connectivity-proofs.md)). A third setting, `--connectivity none`, drops connectivity entirely (0.10 MB, 0.01 s); it solves a different problem and is the attribution floor rather than a solving mode --- but only at this size. At `h11-1` the same relaxation costs **9.65 GB and 446 479 recursions**, against `propagator-no-cuts`'s 93.7 MB and 4 920 (with the default forcing, 679 MB and 1 932), because dropping connectivity makes the objective much harder; do not read the `h5-1` attribution as a statement about hitori generally. That `h11-1` pair is also the set's cleanest **DB-tax** demonstration, since only the encoding differs: 9.65 GB checks in 779.9 s and the no-cuts 93.7 MB in 626.7 s, so a proof 103× smaller checks only 1.24× faster, the propagator's 141 506-row `.opb` making each line about eighty times dearer to verify. |
| `hitori --dzn h5-1.dzn --connectivity propagator` vs `--connectivity propagator-no-cuts` | One propagator, one instance, with its cut-vertex and bridge forcing on and off --- the difference between generalised arc consistency and one step short of it. The forcing wins on search (24 recursions against 38 here, 1 932 against 4 920 on `h11-1`) and loses on proof (0.25 MB / 0.14 s against 0.13 MB / 0.09 s here, **679 MB / 1 485.3 s against 93.7 MB / 626.7 s** on `h11-1`), because `connected`'s root is existentially quantified and hitori decides it last, so every forcing pays a proof line per candidate root. Kept as a **label-groups candidate**: the cost is large-database RUP penalty on a 141 506-row `.opb`, which is what label groups are meant to remove, and the `-no-cuts` column is the control to measure that against. |
