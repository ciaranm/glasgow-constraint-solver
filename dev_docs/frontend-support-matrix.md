...
| noOverlap (Disjunctive) | `Disjunctive` (1D) / `Disjunctive2D` (2D)[^disj] | ✓ (1D + 2D `diffn`) | ✓ (1D + 2D) | ? |
...
- [#146](https://github.com/ciaranm/glasgow-constraint-solver/issues/146) — `Disjunctive`: 1D shipped (variable starts, constant lengths, strict and non-strict). `Disjunctive2D` (2D `noOverlap` / `diffn`) shipped: variable origins, constant sizes, strict and non-strict, pairwise time-table strength, fully VeriPB-certified. Variable lengths/sizes, k-D, optional tasks, and a sweep / cumulative-relaxation propagator are open follow-ups under the same issue.
