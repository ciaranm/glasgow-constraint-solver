...
2. A solver configuration (`glasgow.msc`, generated by CMake) that
   registers `fzn-glasgow` as a MiniZinc solver.
...
The solver configuration is generated from `minizinc/glasgow.msc.in`,
into two places:

| File                                             | Paths    | For                                                              |
|--------------------------------------------------|----------|------------------------------------------------------------------|
| `<build>/glasgow.msc`, next to `fzn-glasgow`     | absolute | running this build tree; ctest uses it too                       |
| `<prefix>/share/minizinc/solvers/glasgow.msc`    | relative | an install, with `mznlib/` in `<prefix>/share/minizinc/glasgow/` |

So `minizinc --solver build/glasgow.msc` runs the `fzn-glasgow` that
build made, from any working directory, and uses the source tree's
`mznlib/`, so an edit to a `.mzn` there takes effect without rebuilding.
The installed copy's paths are relative to itself, the layout of the
solvers in MiniZinc's own bundle, so an install can be moved or given
its prefix at install time (`cmake --install build --prefix ...`). With
an absolute `CMAKE_INSTALL_BINDIR` or `CMAKE_INSTALL_DATADIR` the paths
are absolute instead, since relative ones would then depend on the prefix.
`install` puts the three pieces in a `minizinc` component, for
`cmake --install build --component minizinc`.

Never hand-write a relative `executable`. MiniZinc resolves it against
the `.msc`'s own directory, but when nothing is there it falls back
**silently** to the working directory and then to `PATH`, and runs
whatever it finds. The old checked-in `glasgow.msc` said
`minizinc/../build/fzn-glasgow`, which is relative to the repository
root, not to the `.msc`, so it always took the fallback and ran whichever
build the shell happened to be sitting next to (issue #977). A relative
`mznlib` that names nothing is an error, not a fallback.
`minizinc-installed-msc` installs into a scratch prefix, moves it, and
solves from a working directory where the relative paths lead nowhere.

`minizinc/glasgow-for-debugging.msc` is still checked in. It declares
the executable as `false`, so MiniZinc never runs a solver, which is
useful for running `fzn-glasgow` by hand under a debugger. It has no path
to get wrong, and its `mznlib` is relative to itself, which is right.
...
Both are declared in the `.msc` files' `extraFlags`, which is what
...
1. Once with `--solver <build>/glasgow.msc` (Glasgow), the generated
   configuration next to the `fzn-glasgow` under test.
...
minizinc/run_installed_msc_test.bash   checks the installed solver definition
```
