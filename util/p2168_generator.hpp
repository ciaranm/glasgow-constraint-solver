// ---------------------------------------------------------------------------
// Vendored into the Glasgow Constraint Solver, with one change. See
// dev_docs/building.md, "The vendored <generator> shim".
//
// Provenance: github.com/lewissbaker/generator, via the fork
// github.com/todoubaba/generator at a849a206f35c2447ab5e605edc5fcf44f673c628
// (which adds a missing <atomic> include and silences some warnings). Boost
// Software License 1.0, reproduced below; the LICENSE file is alongside this
// header.
//
// The change: `std::ranges::elements_of` and the three `yield_value` overloads
// that consume it are REMOVED. libc++ gained its own `std::ranges::elements_of`
// in LLVM 22 (it is absent in 21.x) and exposes it from <ranges>, while still
// not providing <generator> --- so this file's definition became a redefinition
// for anyone on that libc++. The two are not interchangeable either: libc++'s is
// an aggregate with public `range` / `allocator` members where this one has
// `get()` / `get_allocator()`, and its default allocator is
// `std::allocator<std::byte>` where this one's is `use_allocator_arg`, which is
// precisely the distinction the removed overloads used to tell "no allocator"
// from "explicit allocator" apart.
//
// Adapting to libc++'s type therefore means reworking that allocator logic, on a
// facility this tree does not use at all and could not test here. Removing it
// instead keeps the shim small and makes the gap loud: `co_yield
// std::ranges::elements_of(...)` --- yielding a nested generator or range ---
// does not compile against this shim. If you need it, either adapt this file
// properly or reach for a toolchain with a real <generator>. Nothing else is
// changed, and `use_allocator_arg` stays, because `generator`'s own default
// allocator is still spelled with it.
// ---------------------------------------------------------------------------
